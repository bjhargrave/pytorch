...
struct Vectorized<c10::qint8> {
 ...
 public:
  ...
  using vec_internal_unsigned_type = vuint8;
 ...
 public:
  ...
  Vectorized<c10::qint8> C10_ALWAYS_INLINE operator<<(const Vectorized<c10::qint8>& other) const {
    // Shift values < 0 and > sizeof-1 result in 0.
    ...
    return {vec_sl(vec_andc(_vec0, mask0), shift0), vec_sl(vec_andc(_vec1, mask1), shift1)};
  }

  Vectorized<c10::qint8> C10_ALWAYS_INLINE operator>>(const Vectorized<c10::qint8>& other) const {
    // Clamp the shift values such that shift values < 0 and > sizeof-1 are changed to sizeof-1
